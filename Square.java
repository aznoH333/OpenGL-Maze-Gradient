...
public class Square {
    ...
    public void changeColors(float value){
        for(int i = 0;i < colors.length;i += 3){
            colors[i] = (float) (Math.sin((vertices[i]*vertices[i+1])*(value/5)+1))+1/2; //red
            colors[i+1] = (float) (Math.sin((vertices[i]+vertices[i+1])*Math.cos(value/2)+0.5))+1/2; //green
            colors[i+2] = (float) (Math.sin((vertices[i]-vertices[i+1])+value/3)+1)+1/2; //blue
        }
    }
}

//colors[i] = (float) (Math.sin((vertices[i]+vertices[i+1])+value))+1/2; //red
//colors[i+1] = (float) (Math.sin((vertices[i]+vertices[i+1])+value))+1/2; //green
//colors[i+2] = (float) (Math.sin((vertices[i]+vertices[i+1])+value))+1/2; //blue
